# In Your Base assistant proxy

A Cloudflare Worker that stands between the [`hosted/`](../hosted) build of
In Your Base and Anthropic's API. It holds your real Anthropic API key as a
...
1. **Create the Worker.**
   - Dashboard → **Workers & Pages** → **Create** → **Workers** → give it
     a name (e.g. `in-your-base-assistant` — this becomes part of its
...
2. **Create two KV namespaces.**
   - Dashboard → **Storage & Databases** → **KV** → **Create a
     ...
     `in-your-base-usage` and `in-your-base-passcodes`. This namespace
...
8. **Copy the Worker's URL.** It's shown at the top of the Worker's
   overview page: `https://in-your-base-assistant.<your-subdomain>.workers.dev`.
